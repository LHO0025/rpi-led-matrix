...
```bash
chmod +x scripts/setup_writable_data.sh
sudo ./scripts/setup_writable_data.sh
```
...
### Files Modified
- `backend/server.py`: Updated paths to use `/data/matrix`
- `backend/viewer.py`: Updated paths to use `/data/matrix`
