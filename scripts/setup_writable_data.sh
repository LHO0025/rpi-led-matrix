...
# Copy existing data from project directory (if present)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

echo "Copying existing images/config if found..."
if [[ -d "$PROJECT_ROOT/matrix_images" ]]; then
  cp -r "$PROJECT_ROOT/matrix_images"/. "$DATA_DIR/images/" 2>/dev/null || true
  echo "  Copied images from $PROJECT_ROOT/matrix_images"
fi

if [[ -f "$PROJECT_ROOT/config.ini" ]]; then
  cp "$PROJECT_ROOT/config.ini" "$DATA_DIR/config/"
  echo "  Copied config.ini"
fi

if [[ -f "$PROJECT_ROOT/.auth" ]]; then
  cp "$PROJECT_ROOT/.auth" "$DATA_DIR/config/"
  echo "  Copied .auth"
fi
